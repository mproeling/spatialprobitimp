sar_probit_mcmc = function (y, X, W, ndraw = 1000, burn.in = 100, thinning = 1, 
  ...
  for (i in (1 - burn.in):(ndraw * thinning)) {
    ...
    #if(i == (1-burn.in)){
    #  write.table(t(z), "output.z", row.names = F, col.names = F, quote = F, sep = "\t")
    #} else {
    #  write.table(t(z), "output.z", append= TRUE, row.names = F, col.names = F, quote = F, sep = "\t")
    #}
    ...
    if(i == 1 - burn.in){
      write.table(rho, paste0("rhoestimate_nomissing.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
    } else {
      write.table(rho, paste0("rhoestimate_nomissing.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = TRUE)
    }
